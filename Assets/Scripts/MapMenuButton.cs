...
public class MapMenuButton : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.mousePosition.x >= bt.transform.position.x - (130 * SCL) &&
            Input.mousePosition.x <= bt.transform.position.x + (130 * SCL) &&
            Input.mousePosition.y >= bt.transform.position.y - (70 * SCL) &&
            Input.mousePosition.y <= bt.transform.position.y + (70 * SCL) 
            )
        {
